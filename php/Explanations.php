<?php
/**
 * Register Explanations Post Type and Dependencies.
 *
...
class Explanations {
	...
	public function __construct() {
		$this->post_types = PostTypes::get_parsed_post_types();

		$this->screen_ids = array( $this->exp_post_type, "edit-{$this->exp_post_type}" );
	...
	public function register_post_type() {
		register_post_type(
			...
			array(
				'labels'             => array(
					'name'               => __( 'Explanations', 'phpdoc-plugin' ),
					'singular_name'      => __( 'Explanation', 'phpdoc-plugin' ),
					'all_items'          => __( 'Explanations', 'phpdoc-plugin' ),
					'edit_item'          => __( 'Edit Explanation', 'phpdoc-plugin' ),
					'view_item'          => __( 'View Explanation', 'phpdoc-plugin' ),
					'search_items'       => __( 'Search Explanations', 'phpdoc-plugin' ),
					'not_found'          => __( 'No Explanations found', 'phpdoc-plugin' ),
					'not_found_in_trash' => __( 'No Explanations found in trash', 'phpdoc-plugin' ),
				),
				'public'             => false,
			...
			foreach ( $menu as $i => $item ) {
				if ( $menu_slug === $item[2] ) {
					...
					$menu[ $i ][0] = sprintf( // phpcs:ignore
						...
						__( 'Explanations %s', 'phpdoc-plugin' ),
	...
	public function prevent_direct_creation() {
		...
		if ( $post_type && $post_type === $this->exp_post_type ) {
	...
	public function post_to_expl_controls( $post ) {
		...
		<div class="postbox-container" style="margin-top:20px;">
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Explanation', 'phpdoc-plugin' ); ?></h3>
				<div class="inside">
					<table class="form-table explanation-meta">
						<tbody>
						<tr valign="top">
							<th scope="row">
								<label for="explanation-status"><?php esc_html_e( 'Status:', 'phpdoc-plugin' ); ?></label>
							</th>
						...
						<?php if ( $explanation ) : ?>
							<tr valign="top">
								<th scope="row">
									<label for="expl-modified"><?php esc_html_e( 'Last Modified:', 'phpdoc-plugin' ); ?></label>
								</th>
								<td name="expl-modified">
									<p><?php echo esc_html( get_post_modified_time( $date_format, false, $post->ID ) ); ?></p>
								</td>
							</tr>
		...
		<div class="postbox-container" style="margin-top:20px;width:100%;">
			<div class="postbox">
				<div class="inside" style="padding-bottom:0;">
					<strong><?php esc_html_e( 'Associated with: ', 'phpdoc-plugin' ); ?></strong>
					<?php
					printf(
						'<a href="%1$s">%2$s</a>',
						esc_url( get_permalink( $post->post_parent ) ),
						esc_html( str_replace( 'Explanation: ', '', get_the_title( $post->post_parent ) ) )
					);
					?>
				</div>
			</div>
	...
	public function add_roles() {
		add_role(
			'expl_editor',
			__( 'Explanation Editor', 'phpdoc-plugin' ),
			array(
	...
	public function grant_caps( $caps ) {
		...
		if ( in_array( $role, array( 'administrator', 'editor', 'expl_editor' ), true ) ) {
			$base_caps = array(
				'edit_explanations',
	...
	public function expl_row_action( $actions, $post ) {
		if ( ! in_array( $post->post_type, PostTypes::get_parsed_post_types(), true ) ) {
			return $actions;
		}
		...
		if ( $expl ) {
			...
			$expl_action['edit-expl'] = sprintf(
				...
				esc_attr__( 'Edit Explanation', 'phpdoc-plugin' ),
				__( 'Edit Explanation', 'phpdoc-plugin' )
			);
		} else {
			$expl_action['add-expl'] = sprintf(
				...
				esc_attr( $post->ID ),
				__( 'Add Explanation', 'phpdoc-plugin' )
			);
		}
	...
	public function status_controls( $post ) {
		...
		if ( $explanation ) :
			echo wp_kses_post( $this->get_status_label( $explanation->ID ) );
			?>
			<span id="expl-row-actions" class="expl-row-actions">
				<a id="edit-expl" href="<?php echo esc_url( get_edit_post_link( $explanation->ID ) ); ?>">
					<?php esc_html_e( 'Edit Explanation', 'phpdoc-plugin' ); ?>
				</a>
				<?php if ( 'publish' === get_post_status( $explanation ) ) : ?>
					<a href="#unpublish" id="unpublish-expl" data-nonce="<?php echo esc_attr( wp_create_nonce( 'unpublish-expl' ) ); ?>" data-id="<?php the_ID(); ?>">
						<?php esc_html_e( 'Unpublish', 'phpdoc-plugin' ); ?>
					</a>
				<?php endif; ?>
			</span><!-- .expl-row-actions -->
		<?php else : ?>
			<p class="status" id="status-label"><?php esc_html_e( 'None', 'phpdoc-plugin' ); ?></p>
			<span id="expl-row-actions" class="expl-row-actions">
				<a id="create-expl" href="" data-nonce="<?php echo esc_attr( wp_create_nonce( 'create-expl' ) ); ?>" data-id="<?php the_ID(); ?>">
					<?php esc_html_e( 'Add Explanation', 'phpdoc-plugin' ); ?>
	...
	public function get_status_label( $post ) {
		if ( ! $post = get_post( $post ) ) { // phpcs:ignore
			return '';
		}

		switch ( $status = $post->post_status ) { // phpcs:ignore
			case 'draft':
				$label = __( 'Draft', 'phpdoc-plugin' );
				break;
			case 'pending':
				$label = __( 'Pending Review', 'phpdoc-plugin' );
				break;
			case 'publish':
				$label = __( 'Published', 'phpdoc-plugin' );
				break;
			default:
				$status = '';
				$label  = __( 'None', 'phpdoc-plugin' );
				break;
		}
	...
	public function admin_enqueue_base_scripts( $do_enqueue ) {
		return $do_enqueue || in_array( get_current_screen()->id, $this->screen_ids, true );
	}
	...
	public function admin_enqueue_scripts() {
		...
		$parsed_post_types_screen_ids = Functions::get_parsed_post_types_screen_ids();
		...
		if ( in_array(
			...
			wp_enqueue_script( 'wporg-explanations', Plugin::get_plugin_dir( '/assets/js/explanations.js' ), array( 'jquery', 'wp-util' ), '20160630', true );

			wp_localize_script(
				'wporg-explanations',
				'phpdoc-plugin',
				array(
					'editContentLabel' => __( 'Edit Explanation', 'phpdoc-plugin' ),
					'statusLabel'      => array(
						'draft'   => __( 'Draft', 'phpdoc-plugin' ),
						'pending' => __( 'Pending Review', 'phpdoc-plugin' ),
						'publish' => __( 'Published', 'phpdoc-plugin' ),
					),
				)
			);
		}
	...
	public function new_explanation() {
		...
		if ( DevHub\get_explanation( $post_id ) ) {
			wp_send_json_error( new WP_Error( 'post_exists', __( 'Explanation already exists.', 'phpdoc-plugin' ) ) );
		} else {
			...
			if ( ! is_wp_error( $explanation ) && 0 !== $explanation ) {
				...
				wp_send_json_error(
					new WP_Error( 'post_error', __( 'Explanation could not be created.', 'phpdoc-plugin' ) )
				);
			}
		}
	}
	...
	public function un_publish_explanation() {
		...
		if ( $explanation = get_explanation( $post_id ) ) { // phpcs:ignore
			...
			if ( ! is_wp_error( $update ) && 0 !== $update ) {
				...
				wp_send_json_error(
					new WP_Error( 'unpublish_error', __( 'Explanation could not be un-published.', 'phpdoc-plugin' ) )
				);
			}
		}
	}
	...
	public function add_post_column( $columns ) {
		if ( ! empty( $_GET['post_type'] ) && Functions::is_parsed_post_type( $_GET['post_type'] ) ) { // phpcs:ignore
			...
			$col_data = array(
				'has_explanation' => sprintf(
					...
					esc_attr__( 'Has explanation?', 'phpdoc-plugin' ),
					esc_html__( 'Explanation?', 'phpdoc-plugin' )
				),
			);
	...
	public function handle_column_data( $column_name, $post_id ) {
		if ( 'has_explanation' === $column_name ) {
			if ( $explanation = get_explanation( $post_id ) ) { // phpcs:ignore
				printf(
					'<a href="%s">%s%s</a>',
					esc_url( get_edit_post_link( $explanation ) ),
					'<span class="dashicons dashicons-info" aria-hidden="true"></span>',
					'<span class="screen-reader-text">' . esc_html__( 'Post has an explanation.', 'phpdoc-plugin' ) . '</span>'
				);
			}
		}
	}
